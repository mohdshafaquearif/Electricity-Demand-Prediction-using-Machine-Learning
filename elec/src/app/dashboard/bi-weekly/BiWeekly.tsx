...
function BiWeeklyMultiModel() {
  const [startDate, setStartDate] = useState("");
  const [festivals, setFestivals] = useState<boolean[]>(Array(14).fill(false));
  ...
  const getHourlyChartOptions = (data: { [key: string]: ModelData }) => {
    ...
    return {
      title: {
        text: 'Two-Week Hourly Demand Prediction',
        left: 'center',
      },
      tooltip: {
        trigger: 'axis',
  ...
  const getDailyChartOptions = (data: { [key: string]: ModelData }) => {
    ...
    return {
      title: {
        text: 'Daily Total Demand (Two Weeks)',
        left: 'center',
      },
      tooltip: {
        trigger: 'axis',
    ...
    <div className="min-h-screen p-6 bg-gray-100">
      <div className="max-w-7xl mx-auto bg-white p-8 rounded-lg shadow-lg">
        <h1 className="text-4xl font-bold text-gray-800 mb-8 text-center">
          Two-Week Multi-Model Demand Prediction
        </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {Object.entries(modelData).map(([modelType, data]) => (
                <div key={modelType} className="bg-gray-50 p-6 rounded-lg shadow-md">
                  ...
                  <p className="text-lg text-gray-600 mb-4">Total Two-Week Consumption: {formatDemand(data.totalWeeklyDemand)} kWh</p>
...
export default BiWeeklyMultiModel;
